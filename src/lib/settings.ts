...
const UNSAFE_ALLOW_ALL_ENV_KEY = "unsafeAllowAllEnv";
...
export const DEFAULT_UNSAFE_ALLOW_ALL_ENV = false;
...
export async function loadUnsafeAllowAllEnv(): Promise<boolean> {
  const stored = await store.get<unknown>(UNSAFE_ALLOW_ALL_ENV_KEY);
  if (typeof stored === "boolean") return stored;
  return DEFAULT_UNSAFE_ALLOW_ALL_ENV;
}

export async function saveUnsafeAllowAllEnv(value: boolean): Promise<void> {
  await store.set(UNSAFE_ALLOW_ALL_ENV_KEY, value);
  await store.save();
}
